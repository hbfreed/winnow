...
from winnow.runtime import WinnowAfmoeForCausalLM
from winnow.scoring import channel_scores, reap_scores
from winnow.selection import select_channels


def test_afmoe_collection_and_channel_checkpoint(tmp_path, tiny_afmoe, input_ids):
    adapter = adapter_for(tiny_afmoe)
    assert adapter.family == "afmoe"
    assert adapter.sigmoid_router
    # Layer 0 is dense, so only the two sparse layers are adapted.
    assert [index for index, _block in adapter.layers] == [1, 2]
    with StatsCollector(tiny_afmoe) as collector, torch.no_grad():
        tiny_afmoe(input_ids, use_cache=False)
    ...
    assert isinstance(loaded, WinnowAfmoeForCausalLM)

    source_shared = tiny_afmoe.model.layers[1].mlp.shared_experts.down_proj.weight
    ...
    source_dense = tiny_afmoe.model.layers[0].mlp.down_proj.weight
    loaded_dense = loaded.model.layers[0].mlp.down_proj.weight
    torch.testing.assert_close(loaded_dense, source_dense)
    for layer in (1, 2):
        block = loaded.model.layers[layer].mlp
        assert block.expert_bias.shape[0] == block.router.gate.weight.shape[0]
    with torch.no_grad():
...
def test_afmoe_keep_one_checkpoint_matches_source(tmp_path, tiny_afmoe, input_ids):
    ...
    for layer in (1, 2):
        torch.testing.assert_close(
            loaded.model.layers[layer].mlp.expert_bias,
            tiny_afmoe.model.layers[layer].mlp.expert_bias,
        )
    with torch.no_grad():
        expected = tiny_afmoe(input_ids, use_cache=False).logits
