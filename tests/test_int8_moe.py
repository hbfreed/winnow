"""Contracts for the packed INT8 W8A16 quantization and the Laguna fast block."""
...
from winnow.runtime.fast import FastLagunaMoE, FastOlmoeMoE
...
def test_fast_laguna_int8_matches_bf16():
    torch.manual_seed(9)
    device = torch.device("cuda")
    module = FastLagunaMoE(128, [128, 256, 128], 2, routed_scaling_factor=2.5)
...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA is not available")
def test_fast_laguna_matches_reference_math():
    torch.manual_seed(11)
    device = torch.device("cuda")
    module = FastLagunaMoE(128, [128, 128], 2, routed_scaling_factor=2.5)
    module.shared_experts = torch.nn.Linear(128, 128, bias=False)
    _filled(module)
    with torch.no_grad():
